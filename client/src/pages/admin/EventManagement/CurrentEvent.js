...
export default function CurrentEvent() {
  ...
  const { current_event, getCurrentEvent, assignSatelliteTable, makeTable, loading } = useDraw();
  ...
  let mainflag = current_event ? current_event.satellite.filter(item => item.status == false).length : 0;
        ...
        <TableContainer>
          <Table>
            ...
            <TableBody>
                  ...
                  <TableCell>
                    {
                      loading ? "Loading..." : (
                        ...
                        }} onClick={() => {makeTable()}} disabled={current_event.status !== 0 || mainflag == 0} > Draw </Button>
                      )
                    }
                  </TableCell>
                </TableRow> :
              ...
              {
                current_event ? current_event.satellite.length > 0 ?
                  current_event.satellite.map((satellite, index) => {
                    return <TableRow key={index}>
                      ...
                      <TableCell>
                        <Button onClick={() => handleOpen(satellite._id)} varient="contained" sx={{
                          backgroundColor: 'yellow', color: '#000',
                          '&:hover': { backgroundColor: '#29B2FE', color: '#fff' }
                        }} disabled={!satellite.status}> Draw </Button>
                      </TableCell>
                    </TableRow>
                  })
                  :
                ...
                >
                  {
                    roomname.map((item, index) => (
                      <MenuItem value={index} key={index}>{roomname[index]}</MenuItem>
                    ))
                  }
                </Select>
              </FormControl>
