...
export default function SateliteEvent() {
  const { isCreatedEvent, current_event, create_sEvent, getCurrentEvent, days, getAllDays  } = useDraw();
  ...
  let buttonFlag = current_event.length == 0 ? false : current_event.status == 0 && current_event.main.price != 0 ? false : true;
              ...
              <Grid item xs={12} md={12}>
                <MotionInView variants={varFadeInUp}>
                  <form className="form" onSubmit={onSubmit}>
                    ...
                    />
                    <Button onClick={onSubmit} disabled={buttonFlag} variant="contained" sx={{mt: 3}} > SUBMIT </Button>
                  </form>
                </MotionInView>
              </Grid>
            </Grid>
